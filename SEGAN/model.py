...
class VirtualBatchNorm1d(Module):
    """
    Virtual Batch Normalization for 1D conv features.
    Stable numerics: clamps variance, gamma/beta init.
    Expects inputs [B, C, L].
    """
    ...
    def __init__(self, num_features, eps=1e-5):
        ...
        self.gamma = Parameter(torch.ones(1, num_features, 1))
        self.beta = Parameter(torch.zeros(1, num_features, 1))

    def get_stats(self, x):
        mean = x.mean(2, keepdim=True).mean(0, keepdim=True)      # [1, C, 1]
    ...
    def __repr__(self):
        return f"{self.__class__.__name__}(num_features={self.num_features}, eps={self.eps})"


class Generator(nn.Module):
    """Generator (SEGAN U-Net style)"""

    def __init__(self):
        super().__init__()
        # encoder
        self.enc1 = nn.Conv1d(1, 16, 32, 2, 15)
        ...
        # decoder
        self.dec10 = nn.ConvTranspose1d(2048, 512, 32, 2, 15)
    ...
    def forward(self, x, z):
        ...
        encoded = torch.cat((c, z), dim=1)  # expect shapes to match
...
class Discriminator(nn.Module):
    """Discriminator with Virtual Batch Norm and dynamic FC sizing"""

    def __init__(self):
        super().__init__()
        negative_slope = 0.03

        self.conv1 = nn.Conv1d(2, 32, 31, 2, 15)
        ...
        # fc created dynamically on first forward
    ...
    def forward(self, x, ref_x):
        # reference pass collect stats
        ref = self.conv1(ref_x)
        ref, mean1, meansq1 = self.vbn1(ref, None, None)
        ref = self.lrelu1(ref)

        ref = self.conv2(ref)
        ref, mean2, meansq2 = self.vbn2(ref, None, None)
        ref = self.lrelu2(ref)

        ref = self.conv3(ref)
        ref = self.dropout1(ref)
        ref, mean3, meansq3 = self.vbn3(ref, None, None)
        ref = self.lrelu3(ref)

        ref = self.conv4(ref)
        ref, mean4, meansq4 = self.vbn4(ref, None, None)
        ref = self.lrelu4(ref)

        ref = self.conv5(ref)
        ref, mean5, meansq5 = self.vbn5(ref, None, None)
        ref = self.lrelu5(ref)

        ref = self.conv6(ref)
        ref = self.dropout2(ref)
        ref, mean6, meansq6 = self.vbn6(ref, None, None)
        ref = self.lrelu6(ref)

        ref = self.conv7(ref)
        ref, mean7, meansq7 = self.vbn7(ref, None, None)
        ref = self.lrelu7(ref)

        ref = self.conv8(ref)
        ref, mean8, meansq8 = self.vbn8(ref, None, None)
        ref = self.lrelu8(ref)

        ref = self.conv9(ref)
        ref = self.dropout3(ref)
        ref, mean9, meansq9 = self.vbn9(ref, None, None)
        ref = self.lrelu9(ref)

        ref = self.conv10(ref)
        ref, mean10, meansq10 = self.vbn10(ref, None, None)
        ref = self.lrelu10(ref)

        ref = self.conv11(ref)
        ref, mean11, meansq11 = self.vbn11(ref, None, None)

        # train pass using ref stats
        x = self.conv1(x)
        ...
        x = self.conv_final(x)
        x = self.lrelu_final(x)  # [B,1,T]

        x = x.view(x.size(0), -1)  # safe flatten

        # dynamic fc creation
        if (self.fully_connected is None) or (x.size(1) != self.fully_connected.in_features):
            fc = nn.Linear(x.size(1), 1).to(x.device)
